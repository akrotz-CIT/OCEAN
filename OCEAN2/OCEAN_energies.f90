...
!> brief Makes the core-level allow array using a Fermi-factor
  subroutine core_allow_ff(  sys, allowArray, ener, metal, efermi, temp )
    ...
    integer :: i, j, k, n(3)
    logical :: have_allowarray
    if( present( temp ) ) then
      itemp = 1.0_dp / temp
      scaledFermi = - efermi / temp
    endif
    inquire(file='allowarray.dat',exist=have_allowarray)
    ...
    n = shape( ener )
    if(have_allowarray) then
    open(unit=98,file='allowarray.dat',form='formatted',status='unknown')
    do k = 1, n(3)
       do j = 1, n(2)
          do i = 1, n(1) 
               !ALEX COMMENT BELOW TO REVERT
                read(98, *) allowArray(i,j,k)
          enddo
       enddo
    enddo
    close(98) 
        ...
        do j = 1, n(2)
          do i = 1, n(1)
            if( ener( i, j, k ) .ge. efermi ) then
              !ALEX UNCOMMENT BELOW TO REVERT
             if(.not. have_allowarray)  allowArray(i,j,k) = con
            else
!              if( myid .eq. root ) write(6,*) i,j,k, ener(i,j,k)
                !ALEX UNCOMMENT BELOW TO REVERT
             if(.not. have_allowarray)  allowArray(i,j,k) = val
            endif
          enddo
        enddo
      enddo
    ...
    close(99)
    if(.not. have_allowarray) then
        open(unit=99,file='allowarray.dat',form='formatted',status='unknown')
        do k = 1,n(3)
           do j = 1,n(2)
               do i = 1,n(1)
                write(99,*) allowarray(i,j,k)
                enddo
           enddo
        enddo
        close(99)
     endif
     open(unit=99,file='energy_allow.dat',form='formatted',status='unknown')
        do k = 1,n(3)
        do j = 1,n(2)
        do i = 1,n(1)
        write(99,*) ener(i,j,k), allowarray(i,j,k)
        enddo
        enddo
        enddo
